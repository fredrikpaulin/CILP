  ...
  clause.body.forEach((goal, depth) => {
    ...
    if (heads.has(goal.predicate)) {
      ...
      goal.args.forEach((a, i) => {
        if (m[i] !== "out") return
        const slot = `${tuple}[${oi++}]`
        if (a.type === "var" && !bound.has(a.name)) { lines.push(`${ind}  const ${jsVar(a.name)} = ${slot}`); bound.add(a.name) }
        // already bound (or a constant): the returned value must equal it — a constraint.
        else lines.push(`${ind}  if (!termEqual(${slot}, ${renderTerm(a)})) continue`)
      })
    } else {
      const reads = []
...
export function lowerJavaScript(program, harness, options = {}) {
  ...
  const source = [
    ...
    `import { makeRegistry, applySubstitution, termEqual } from ${JSON.stringify(core)}`,
